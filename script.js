document.addEventListener("DOMContentLoaded", function() {
    ...
    function generateCalendar(selectedYear, isYearSelected) {
        ...
        if (isYearSelected && selectedYear) {
            // 确保使用中国时区的年初和年末
            startDate = toChinaTime(new Date(Date.UTC(selectedYear, 0, 1)));
            endDate = toChinaTime(new Date(Date.UTC(selectedYear, 11, 31)));
        } else {
            endDate = today;
            startDate = toChinaTime(new Date(endDate));
            startDate.setDate(startDate.getDate() - 364); // 从今天往前364天
        }

        // 确保从当前日期的周日开始生成方格，同时保持中国时区
        let currentDate = toChinaTime(new Date(startDate));
